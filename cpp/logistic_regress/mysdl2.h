...
struct Bitmap {
  ...
  void blit(Bitmap &dstBmp, const Rect *srcRect = nullptr, Rect *dstRect =
                nullptr);
};
...
struct SDL {
  ...
  bool init(Uint32 w, Uint32 h, bool borderless = true, std::string_view title =
                "demo",
            bool withOpenGL = false);
  void pump();
  void swap();
  void term();
  Uint32 getTicks();
  ...
  void takeScreenshot();
  void loadFont(std::string name, std::string fontFile, int fontSize);
};
